...
`src/data/navigation.ts` がヘッダーナビゲーションの唯一の定義元です。入城案内の状態、日時、招待URLは `src/data/join.ts` で管理します。`inviteUrl` が空、または受付停止中なら、ページにDiscordリンクは表示されません。管理者が外部公開を承認した招待URLのみ、`src/data/join.ts` で一元管理して掲載できます。
...
未承認の原稿は `draft` または `review` にし、公開済みに見える内容を記載しないでください。サンプルの制度、階級、権限、歴史、個人情報、Discord内部情報は追加してはいけません。管理者が公開を承認した招待URLだけは、上記の一元管理方針に従って掲載できます。
